...
/* ======================================================
   SPREADSHEET API CONFIG
====================================================== */
const SPREADSHEET_API_URL = "https://script.google.com/macros/s/AKfycbyvuSQXHURAIbPomKevF0v8amkuRJKNuGETJzKGPb9P6mkHVbxYsVAZG_Bkl8MT5vds/exec";

/* ======================================================
   LOAD DATA FROM SPREADSHEET (SOURCE OF TRUTH)
====================================================== */

async function loadPesertaFromSheet() {
    peserta = await fetch(
        `${SPREADSHEET_API_URL}?action=getPeserta`
    ).then(r => r.json());
}

async function loadHadiahFromSheet() {
    hadiah = await fetch(
        `${SPREADSHEET_API_URL}?action=getHadiah`
    ).then(r => r.json());

    // simpan snapshot awal untuk reset
...
async function loadPemenangFromSheet() {
    kandidat = await fetch(
        `${SPREADSHEET_API_URL}?action=getPemenang`
    ).then(r => r.json());
}
/* ======================================================
   UTIL
...
async function init() {
    try {
        await Promise.all([
            loadPesertaFromSheet(),
            loadHadiahFromSheet(),
            loadPemenangFromSheet()
        ]);

        renderAll(); // 🔥 render SETELAH data siap
    } catch (err) {
        console.error(err);
...
/* ======================================================
   Synchronize Data
====================================================== */
async function syncToSpreadsheet(action, rows) {
...
async function syncPesertaToSheet(peserta) {
  const res = await fetch(SPREADSHEET_API_URL, {
    method: "POST",
    headers: {
  ...
  const json = await res.json();
  console.log("SYNC PESERTA:", json);

  if (!json.success) {
    alert("Gagal sync peserta: " + json.error);
  }
}

function syncHadiahToSheet() {
  if (!hadiah.length) return;

  const form = new URLSearchParams();
  form.append("action", "saveHadiah");
  form.append("data", JSON.stringify(hadiah));

  return fetch(SPREADSHEET_API_URL, {
    method: "POST",
    body: form
  }).then(r => r.json());
}

function syncPemenangToSheet(rows) {
  if (!rows || !rows.length) return;

  const form = new URLSearchParams();
  form.append("action", "savePemenang");
        ...
        setTimeout(() => {
            ...
            if (jumlah === 1) {
                ...
                requestAnimationFrame(() => {
                    ...
                    saveAll();
                    syncPemenangToSheet([kandidatBaru[0]]);
...
//     peserta = rows.map(r => ({
//       bib: String(r.BIB || r.bib).trim(),
//       nama: String(r.Nama || r.nama || "-").trim()
//     }));

//     await syncToSpreadsheet("savePeserta", peserta);
//     alert("Peserta berhasil disimpan ke Spreadsheet");
//   };
...
//     hadiah = rows.map(r => ({
//       prize: String(r.Prize || r.prize).trim(),
//       stock: parseInt(r.Stock || r.stock || 0)
//     }));

//     await syncToSpreadsheet("saveHadiah", hadiah);
//     alert("Hadiah berhasil disimpan ke Spreadsheet");
//   };
...
// ================= CEK UNDIAN (PUBLIK) =================
function cekUndian() {
    const bib = document.getElementById("cekBibInput").value.trim();
    ...
    fetch(`${SPREADSHEET_API_URL}?action=cekUndian&bib=${bib}`)
        .then(r => r.json())
        .then(res => {
            if (res.status === "win") {
                resultEl.innerHTML = `
                    <div class="success">
                        🎉 Selamat ${res.nama}!<br>
                        Anda memenangkan <b>${res.hadiah}</b>
                    </div>
                `;
            } else {
